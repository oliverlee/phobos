...
namespace {
    ...
    constexpr float K_P_drive = 0.714; // motor drive proportional gain
...
int main(void) {
    ...
    printfq("running opik %.7s, k = %8.3f, m = %8.3f\r\n", g_GITSHA1, k, m_star);
    printfq("fields are:\n\r");
    ...
    while (true) {
        ...
        const float steer_acceleration = (kistler_torque - k*steer_angle)/m_star_lower;

        // perform integration to determine drive velocity command
        steer_velocity += static_cast<float>(ST2US(dt))/1000000.0f*steer_acceleration;

        // get feedforward velocity command
        const float velocity_ff = steer_acceleration*(m_upper + m_lower)/K_P_drive;

        const float feedback_velocity = steer_velocity + velocity_ff;
        sa::set_kollmorgen_velocity(feedback_velocity);
